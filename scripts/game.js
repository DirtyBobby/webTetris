...
function showTetramino() {
  for (let i = 0; i < 4; i++) {
    if (tetramino.figure[i].x >= 0 && tetramino.figure[i].y >= 0) {
      field[tetramino.figure[i].x][tetramino.figure[i].y].type =
        tetramino.figureType;
...
function isCollision(_direction) {
  if (_direction == -1) {
    ...
    for (let i = 0; i < 4; i++) {
      if (
        tetramino.figure[i].x <= 0 ||
        (tetramino.figure[i].y >= 0 &&
          field[tetramino.figure[i].x - 1][tetramino.figure[i].y].type !=
            TileType.empty &&
          field[tetramino.figure[i].x - 1][tetramino.figure[i].y]
            .isFallingBlock == false)
      ) {
        return false;
      }
    ...
    for (let i = 0; i < 4; i++) {
      if (
        tetramino.figure[i].y >= 0 &&
        (tetramino.figure[i].y >= 19 ||
          (field[tetramino.figure[i].x][tetramino.figure[i].y + 1].type !=
            TileType.empty &&
            field[tetramino.figure[i].x][tetramino.figure[i].y + 1]
              .isFallingBlock == false))
      ) {
        return false;
      }
...
function showMenuTetramino() {
  ...
  for (let i = 0; i < 4; i++) {
    menuFieldCurrent[currentTetramino.figure[i].x][
      currentTetramino.figure[i].y
    ].type = currentTetramino.figureType;
    menuFieldNext[nextTetramino.figure[i].x][nextTetramino.figure[i].y].type =
      nextTetramino.figureType;
  }
...
function rotateFallingBlock() {
  ...
  for (let i = 0; i < 4; i++) {
    if (tetramino.figure[i].x < 0 || tetramino.figure[i].x > 9) {
      ...
      tetramino.figure[i].y >= 0 &&
      tetramino.figure[i].y < 20 &&
      field[tetramino.figure[i].x][tetramino.figure[i].y].type !=
        TileType.empty &&
      field[tetramino.figure[i].x][tetramino.figure[i].y].isFallingBlock ==
        false
    ) {
      _isRotationAvailable = false;
      break;
...
function sendScore(score_num) {
  ...
  */
  fetch("saveScore.php", {
    method: "POST",
    body: { score: score_num },
  })
    .then((responce) => alert(responce.body["message"]))
    .then(() => {
